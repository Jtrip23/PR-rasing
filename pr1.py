...
def create_pull_request(username, token, repo_name, base_branch, compare_branch, reviewers):
    ...
    try:
        ...
        pr = repo.create_pull(
            ...
            title=f"Merge {compare_branch} into {base_branch}",
...
def process_excel_file(username, token, excel_file, output_excel_file):
    ...
    try:
        ...
        required_columns = {'source_repo_name', 'base_branch', 'compare_branch', 'reviewers'}
        ...
        for index, row in df.iterrows():
            ...
            compare_branch = row['compare_branch']
            ...
            logger.debug(f"Processing repository: {repo_name}, base_branch: {base_branch}, compare_branch: {compare_branch}, reviewers: {reviewers}")

            pr_url = create_pull_request(username, token, repo_name, base_branch, compare_branch, reviewers)
